...
export const authRegisterUser =
  ({ login, email, password, avatar }: INewUser) =>
  async (dispatch: (arg0: { payload: IUser; type: 'auth/updateUserProfile' }) => void, _: any) => {
    try {
      const user = await createNewUser(login, email, password, avatar);
      if (user) dispatch(authSlice.actions.updateUserProfile(<IUser>user));
    } catch (error) {
...
export const authLogOutUser =
  () =>
  async (
    dispatch: (arg0: { payload: { stateChange: boolean }; type: 'auth/signOutUser' }) => void,
  ) => {
    try {
      await logOutUser();
...
export const authStateChangeUser =
  ...
  async (
    dispatch: (arg0: {
      payload: IUser | { stateChange: boolean };
      type: 'auth/updateUserProfile' | 'auth/authStateChange';
    }) => void,
  ) => {
...
export const authUpdateUserPhoto = (avatar: string) => {
  return async (
    dispatch: (arg0: { payload: IUser; type: 'auth/updateUserProfile' }) => void,
    _: any,
  ) => {
    try {
