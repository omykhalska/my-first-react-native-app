...
export const createNewUser = async (login: string, email: string, password: string, avatar: string) => {
    try {
...
export const getAllComments = async (postId: string, setComments: (comments: IComment[]) => void) => {
    try {
...
export const uploadPhotoToServer = async ({photoUrl, photoDir}: { photoUrl: string, photoDir: string }) => {
    try {
        const id = uuid.v4();
